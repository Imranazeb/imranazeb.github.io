...
from database import (
    ...
    stock_exists,
    stock_reviewed_already,
    update_reviewed_date,
)
...
def create_blog_post(result):
    ...
    for mover in other_stocks:
        ...
        content_parts.append(
            f"- **{symbol_text}** ({mover.get('shortName', 'N/A')}): +{percent_change:.2f}%"
        )
...
if result:
    ...
    if post_file:
        ...
        if not stock_exists(stock_data.symbol):
            ...
            update_reviewed_date(stock_data.symbol)
            logger.info(
                f"Stock {stock_data.symbol} already exists in database, updated last reviewed timestamp"
            )
else:
